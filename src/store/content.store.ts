...
export interface ContentActions {
  ...
  saveFooter: (data: FooterRequest) => Promise<boolean>;
...
export const useContentStore = create<ContentState & ContentActions>((set, get) => ({
  ...
  loadCategories: async () => {
    ...
    try {
      // Categories are loaded from PublicService in properties store
      // This is a placeholder - would need actual content management endpoints
      set({ 
  ...
  uploadPhotos: async (files: File[]) => {
    set({ loading: { ...get().loading, uploading: true } });
    
    try {
      const results = await bulkUploadPhotos(files);
      await get().loadPhotos();
      
      set({ loading: { ...get().loading, uploading: false } });
      return true;
    } catch (error: any) {
      set({ 
        loading: { ...get().loading, uploading: false },
        errors: { ...get().errors, uploadPhotos: error.message },
      });
      return false;
    }
  },
